...
export default function ClientDetailsPage() {
  ...
  // Check if we have updated client data in sessionStorage and use it
  useEffect(() => {
    if (clientId && typeof window !== "undefined") {
      const updatedClientStr = sessionStorage.getItem(`client_${clientId}_updated`);
      if (updatedClientStr) {
        // Clear it after using
        sessionStorage.removeItem(`client_${clientId}_updated`);
      ...
      <div className="max-w-[1200px] mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="bg-white rounded-2xl border border-gray-200 p-6 mb-6">
          <div className="flex items-start justify-between mb-4">
            ...
            <button
              onClick={() => router.push(`/admin/dashboard/clients/${client.id}/configuration`)}
              className="px-5 py-2.5 rounded-xl bg-green-600 text-white hover:bg-green-700 transition-colors"
            >
              Edit Client
            </button>
          </div>
