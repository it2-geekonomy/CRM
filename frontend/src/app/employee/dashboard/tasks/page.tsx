...
export default function EmployeeTaskCalendarPage() {
  ...
  const { data: tasksData, isLoading, isError, error, refetch: refetchTasks } = useGetCalendarTasksQuery(calendarQueryParams);
    ...
    const timer = setTimeout(() => {
      ...
      rows.forEach((row) => {
        ...
        dayBgs.forEach((bg, index) => {
          ...
          // Add click handler to date cell
          if (dateLink) {
            dateLink.style.cursor = 'pointer';
            dateLink.onclick = (e) => {
              e.preventDefault();
              e.stopPropagation();
              setSelectedDate(new Date(cellDate));
            };
          }
          
          // Also add click handler to the background
  ...
  const handleCreateTask = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      // Refetch tasks to show the new task on the calendar
      // RTK Query should auto-refetch due to cache invalidation, but we'll also manually refetch
      setTimeout(async () => {
        try {
          await refetchTasks();
        } catch (error) {
            ...
            <>
              <div className="overflow-x-auto -mx-4 sm:mx-0">
                <div className="min-w-[600px] sm:min-w-0 px-4 sm:px-0">
                  <Calendar
                    key={`calendar-${currentDate.getTime()}-${calendarTasks.length}`}
                    localizer={localizer}
                    events={[]}
                    startAccessor="start"
                    endAccessor="end"
